...
class BelongsToMany extends Relation {
    ...
    createPivotRecord(data, record, related) {
        related.forEach((id) => {
            ...
            const dataPivotKey = Object.assign(
                {},
                pivotRecord,
                pivotData,
                {
            ...
            data[this.pivot.entity] = Object.assign(
                {},
                data[this.pivot.entity],
                {
                    [pivotKey]: dataPivotKey
                }
            );
        });
    }
...
async function create$1(context, payload) {
    const state = context.state;
    const entity = state.$name;
    return context.dispatch(`${state.$connection}/create`, Object.assign({}, payload, entity), { root: true });
}
/**
...
async function insert$2(context, payload) {
    const state = context.state;
    const entity = state.$name;
    return context.dispatch(`${state.$connection}/insert`, Object.assign({}, payload, entity), { root: true });
}
/**
...
async function update$1(context, payload) {
    ...
    return context.dispatch(`${state.$connection}/update`, Object.assign({}, entity, payload), { root: true });
}
/**
...
async function insertOrUpdate$1(context, payload) {
    const state = context.state;
    const entity = state.$name;
    return context.dispatch(`${state.$connection}/insertOrUpdate`, Object.assign({}, entity, payload), { root: true });
}
...
class Query {
    ...
    processUpdate(data, instance) {
        ...
        if (instance.constructor !== this.model && instance instanceof Model) {
            return this.hydrate(Object.assign({}, instance, data), instance.constructor);
        }
     ...
     */
    commit(name, payload) {
        this.store.commit(`${this.database.namespace}/${name}`, Object.assign(
            {},
            {
    ...
    hydrate(record, forceModel) {
        ...
        if (!this.appliedOnBase && record[this.model.typeKey] === undefined) {
            ...
            record = Object.assign({}, record, { [this.model.typeKey]: typeValue });
    ...
    hydrateRecordsByMerging(records) {
        return Object.keys(records).reduce((instances, id) => {
            ...
            if (modelForRecordInStore === null) {
                instances[id] = this.hydrate(Object.assign({}, recordInStore, record));
                return instances;
            }
            instances[id] = this.hydrate(Object.assign({}, recordInStore, record), modelForRecordInStore);
            return instances;
        }, {});
    }
    /**
...
class Connection {
     ...
     */
    insert(record) {
        this.state.data = Object.assign({}, this.state.data, { [record.$id]: record });
    }
    /**
     * Insert the given records.
     */
    insertRecords(records) {
        this.state.data = Object.assign({}, this.state.data, records);
    }
    /**
...
class Database {
    ...
    createSubGetters(entity) {
        return Object.assign({}, Getters, entity.module.getters);
    }
    /**
     * Create sub actions.
     */
    createSubActions(entity) {
        return Object.assign({}, Actions, entity.module.actions);
    }
    /**
